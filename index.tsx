...
const zoomOverlay = document.getElementById('zoomOverlay') as HTMLDivElement;

// API Key Modal elements
const settingsButton = document.getElementById(
  'settingsButton',
) as HTMLButtonElement;
const apiKeyModalOverlay = document.getElementById(
  'apiKeyModalOverlay',
) as HTMLDivElement;
const closeApiKeyModalButton = document.getElementById(
  'closeApiKeyModalButton',
) as HTMLButtonElement;
...
let isLocationBased: boolean = false;
const editIconSVG = `
...
} = {element: null, parent: null, nextSibling: null};
...
function renderFlashcards() {
  ...
  currentFlashcards.forEach((flashcard, index) => {
    ...
    termDiv.innerHTML = DOMPurify.sanitize(
      marked.parse(flashcard.term || '') as string,
    );
    ...
    definitionDiv.innerHTML = DOMPurify.sanitize(
      marked.parse(flashcard.definition || '') as string,
    );
    ...
    if (isMapSearch) {
      const query = `${flashcard.term}, ${currentLocation!.city}, ${
        currentLocation!.country
      }`;
...
async function handleFileSelect(event: Event) {
  ...
  reader.onload = (e) => {
    ...
    try {
      ...
      errorMessage.textContent = `Error importing file: ${
...
function zoomIn(cardElement: HTMLElement) {
  if (zoomedCardState.element) return;

  // Store original position so we can return it later
  zoomedCardState = {
    element: cardElement,
    parent: cardElement.parentElement as HTMLElement,
...
flashcardsContainer.addEventListener('click', (e) => {
  const card = (e.target as HTMLElement).closest('.flashcard:not(.editing)');
  if (card) {
    zoomIn(card as HTMLElement);
  }
});
